...
{else}
	...
	<h3 class="sr-only">{t}Table of contents{/t}</h3>
	<div class="nav flex-column nav-pills">
		...
		{foreach $book->getChapters() as $ch}
			...
			{foreach $ch->getSubchapters() as $sub_ch}
				<a href="{link_to action=detail id=$sub_ch}" class="nav-link {if USING_BOOTSTRAP3 || USING_BOOTSTRAP4}pl-4{else}ps-4{/if}{if $current_chapter && $current_chapter->getNo()===$sub_ch->getNo()} active{/if}">{$sub_ch->getNo()}. {$sub_ch->getTitle()}</a>
			{/foreach}
		{/foreach}
	</div>
